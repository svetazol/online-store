<template>
  <v-dialog v-model="dialog" @keydown.esc="$router.push({path: '/'})" persistent
          ...
          <v-card class="px-4">
            <v-card-text>
              <v-form ref="loginForm" v-model="valid" lazy-validation>
                <v-row justify="center" class="red--text" v-if="errorMessage">
                  {{ errorMessage }}
                </v-row>
                <v-row>
                  <v-col cols="12">
  ...
  methods: {
    async validate() {
      ...
      if (this.$refs.loginForm.validate()) {
        ...
        bodyFormData.set('password', this.loginPassword)
        bodyFormData.set('login', this.loginEmail)
        try {
